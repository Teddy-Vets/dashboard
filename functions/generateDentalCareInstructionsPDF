...
Deno.serve(async (req) => {
    try {
        ...
        const instructions = await base44.entities.DentalCareInstructions.filter({ id: instruction_id });
        ...
        const pageWidth = doc.internal.pageSize.getWidth();
        let y = createPageHeader(doc, clinicName, 'הנחיות טיפול שיניים');
        ...
        y = writeSection(doc, 'תאריך הטיפול:', formatHebrewDate(instruction.procedure_date), y, pageWidth);

        y += 5;

        if (instruction.post_op_rest_instructions) {
            y = checkPageBreak(doc, y);
            y = writeSection(doc, 'הנחיות למנוחה לאחר הפרוצדורה:', instruction.post_op_rest_instructions, y, pageWidth);
        }
        ...
        if (instruction.local_care_instructions) {
            y = checkPageBreak(doc, y);
            y = writeSection(doc, 'טיפול מקומי:', instruction.local_care_instructions, y, pageWidth);
        }

        if (instruction.preventative_care_details) {
            y = checkPageBreak(doc, y);
            y = writeSection(doc, 'פרטי טיפול מונע:', instruction.preventative_care_details, y, pageWidth);
        }
        ...
        return new Response(pdfBytes, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename=dental-care-instructions-${instruction.pet_name || 'pet'}.pdf`
            }
        });

    } catch (error) {
        console.error('[generateDentalCareInstructionsPDF] Error:', error);
