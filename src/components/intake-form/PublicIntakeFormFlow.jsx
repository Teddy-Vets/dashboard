...
import VisitInfoStep from "./VisitInfoStep";
import ReviewStep from "./ReviewStep";

import { submitIntake } from "@/functions/submitIntake";

const STEPS = [
  { id: 1, title: "פרטים אישיים", component: PersonalInfoStep },
  { id: 2, title: "פרטי חיית המחמד", component: PetInfoStep },
  { id: 3, title: "היסטוריה רפואית", component: MedicalHistoryStep },
  { id: 4, title: "מידע על הביקור", component: VisitInfoStep },
  { id: 5, title: "סיכום ואישור", component: ReviewStep }
];

export default function PublicIntakeFormFlow({ token, formId, initialData = {} }) {
  ...
  const handleFileUpload = async (event, fieldName) => {
    ...
    try {
      ...
      alert(`שגיאה בהעלאת הקובץ: ${error.message || 'אנא נסה שנית'}`);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('[PublicIntakeFormFlow] Submitting with formId:', formId);
      ...
      console.error("[PublicIntakeFormFlow] Error submitting intake form:", error);
      setSubmitError(error.message || "אירעה שגיאה בשליחת הטופס. אנא נסה שנית.");
    } finally {
            ...
            <CardContent className="p-12 text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-600 leading-relaxed text-lg">
                  תודה רבה על מילוי טופס ההיכרות.<br />
                  נציג מהמרפאה יצור עמכם קשר בהקדם.
                </p>
              </div>

              <div className="bg-gradient-to-r from-blue-50 to-teal-50 p-6 rounded-lg border-2 border-blue-200">
                <p className="text-slate-700 font-medium">
                  שמרנו את כל הפרטים שמסרתם ונדאג לטפל בכם בצורה הטובה ביותר! 💙
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-1 text-center text-white">
              <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-1 drop-shadow-lg">
                ברוכים הבאים, אנחנו כבר מחכים להכיר
              </h1>
