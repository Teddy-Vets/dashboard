...
import PersonalInfoStepAr from "./PersonalInfoStepAr";
import PetInfoStepAr from "./PetInfoStepAr";
import MedicalHistoryStepAr from "./MedicalHistoryStepAr";
import VisitInfoStepAr from "./VisitInfoStepAr";
import ReviewStepAr from "./ReviewStepAr";

import { submitIntake } from "@/functions/submitIntake";

const STEPS = [
  { id: 1, title: "المعلومات الشخصية", component: PersonalInfoStepAr },
  { id: 2, title: "معلومات الحيوان", component: PetInfoStepAr },
  { id: 3, title: "التاريخ الطبي", component: MedicalHistoryStepAr },
  { id: 4, title: "معلومات الزيارة", component: VisitInfoStepAr },
  { id: 5, title: "المراجعة والإرسال", component: ReviewStepAr }
];

export default function PublicIntakeFormFlowAr({ token, formId, initialData = {} }) {
  ...
  const handleFileUpload = async (event, fieldName) => {
    ...
    try {
      ...
      if (response && response.file_url) {
        ...
        throw new Error("لم يتم استلام رابط من الخادم");
      }
    } catch (error) {
      console.error(`Error uploading file for ${fieldName}:`, error);
      alert(`خطأ في رفع الملف: ${error.message || 'يرجى المحاولة مرة أخرى'}`);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!formId) {
        throw new Error("معرف النموذج مفقود");
      }
      ...
      console.log('[PublicIntakeFormFlowAr] Submitting with formId:', formId);
      ...
      if (response.data && response.data.success) {
        ...
        throw new Error(response.data?.error || "خطأ في إرسال النموذج");
      }
    } catch (error) {
      console.error("[PublicIntakeFormFlowAr] Error submitting intake form:", error);
      setSubmitError(error.message || "حدث خطأ أثناء إرسال النموذج. يرجى المحاولة مرة أخرى.");
    } finally {
  ...
  if (submitSuccess) {
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-teal-50 p-4 flex items-center justify-center">
        <motion.div
            ...
            <CardContent className="p-12 text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-600 leading-relaxed text-lg">
                  شكراً لك على ملء نموذج الاستقبال.<br />
                  سيتصل بك ممثل عن العيادة قريباً.
                </p>
              </div>

              <div className="bg-gradient-to-r from-blue-50 to-teal-50 p-6 rounded-lg border-2 border-blue-200">
                <p className="text-slate-700 font-medium">
                  لقد حفظنا جميع المعلومات التي قدمتها وسنعتني بك بأفضل طريقة! 💙
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-1 text-center text-white">
              <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-1 drop-shadow-lg">
                مرحباً بكم، نحن بالفعل نتطلع للقائكم
              </h1>
              
              <p className="text-xs md:text-sm lg:text-base max-w-3xl mx-auto leading-tight drop-shadow-md">
                عملية الاستقبال سريعة وستساعدنا في إعداد زيارة لطيفة ومخصصة لك ولحيوانك الأليف
              </p>
            </div>
